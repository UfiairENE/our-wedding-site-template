...
The form POSTs a JSON body to a Google Apps Script web app which appends a row to a Google Sheet and handles guest emails. There are two tiers depending on how much you want to automate.
 
---
 
### Tier 1 — Simple (just collect RSVPs)
 
Takes about 10 minutes. Guests submit the form, rows appear in your sheet, and you manage everything else manually.
...
### Tier 2 — Full (seat assignment + automated emails)
 
Use this if you want the sheet to automatically assign seats when you approve a guest and send them confirmation, final week, and day-of emails without you lifting a finger.
 
This is a separate script that lives in its own Google Sheet (not the same one as Tier 1). It ships as `wedding-script.gs` in the [companion repo](https://github.com/UfiairENE/our-wedding-site-template).
 
#### What it does
 
- Accepts RSVP submissions and prevents duplicate registrations by email
- Auto-assigns consecutive seats the moment you mark a guest as `APPROVED` in the sheet
- Sends a **seat confirmation email** immediately on approval (physical guests get their table/seat; virtual guests get the livestream link)
...
- Detects and fixes duplicate seat assignments if they ever occur
#### Sheet setup
 
Create a new Google Sheet with two tabs named exactly `Physical` and `Virtual`. After pasting the script, run **Wedding Admin → Fix Sheet Headers** — it will create the correct 14 columns automatically:
...
| M — Status | **You set this to `APPROVED` to trigger everything** |
| N — Email Sent | Tracked automatically — `YES` / `NO` |
 
#### Script setup
 
1. In your new Google Sheet: **Extensions → Apps Script**
2. Delete all existing code and paste the full contents of `wedding-script.gs`
3. At the top of the file, fill in the `CONFIG` block with your details:
```javascript
var CONFIG = {
  PARTNER_1_NAME   : "Ada",
...
4. **Save**, then **Deploy → New deployment → Web app** (Execute as: Me, Access: Anyone)
5. Copy the Web App URL into your `.env` as `VITE_GOOGLE_SCRIPT_URL` — this replaces the Tier 1 URL
...
These are sent manually when you're ready — open the **Wedding Admin** menu in your sheet and click:
 
- **Send Final Week Reminder** — send ~7 days before the wedding
- **Send Day-Of Email** — send the morning of the wedding
Both emails are only sent to guests who are `APPROVED` and have already received a confirmation (`Email Sent = YES`), so there is no risk of emailing someone twice or emailing unconfirmed guests.
 
#### Reserved seating
 
The script assigns seats automatically. If you need specific guests at a specific table (family, VIPs, etc.), manually type their seat in column L **before** approving them. The script skips any row that already has a seat filled in and will email them with whatever you entered.
